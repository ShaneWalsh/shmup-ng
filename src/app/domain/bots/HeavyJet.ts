...
export class HeavyJet extends BotInstanceImpl{
  ...
  public targetCordsIndex = 0;
    ...
    let range = 10;
    if ((this.posX > (targetCord.targetX - range) && this.posX < (targetCord.targetX + range))
      && (this.posY > (targetCord.targetY - range) && this.posY < (targetCord.targetY + range))) {
      ...
      if ( this.targetCordsIndex >= this.targetCords.length){
		...
		if (this.posY < (targetCord.targetY-range) || this.posY > (targetCord.targetY+range)) {
			this.posY += (this.posY < (targetCord.targetY-range) )? this.posYSpeed:(this.posYSpeed * -1);
		}
		if (this.posX < (targetCord.targetX-range) || this.posX > (targetCord.targetX+range)) {
			this.posX += (this.posX < (targetCord.targetX-range) )? this.posXSpeed:(this.posXSpeed * -1);
		}
